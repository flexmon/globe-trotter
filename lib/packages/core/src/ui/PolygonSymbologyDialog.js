/**
 * PolygonSymbologyDialog.js — Symbology editor for GFB POLYGON and MULTI_POLYGON geometry.
 *
 * Controls: Color mode (Ramp / Discrete), attribute selector, color stops or
 * per-value pickers, fill opacity slider, extrusion toggle + scale.
 *
...
export class PolygonSymbologyDialog {
  ...
  _init() {
    ...
    this._extrusionScale = renderer?.extrusionScale ?? 0;
  ...
  _buildOverlay() {
    ...
    let html = `
            <div class="gt-sym-header">
                <h3 class="gt-sym-title">Polygon Symbology</h3>
    ...
    // ── Fill Color Mode Toggle ──
    html += `
            <div class="gt-sym-section">
                <div class="gt-sym-section-title">Fill Color</div>
    ...
    html += `</div>`; // close fill color section

    // ── Fill Opacity ──
    html += `
            <div class="gt-sym-section">
                <div class="gt-sym-section-title">Fill Opacity</div>
                <div class="gt-sym-row">
    ...
    // ── Extrusion ──
    const extEnabled = this._extrusionScale > 0;
    // Clamp to slider range [0.5×, 5.0×] so the slider value is always valid.
    const extMult = extEnabled ? Math.min(5.0, Math.max(0.5, this._extrusionScale / 0.012)) : 1.0;
    html += `
            <div class="gt-sym-section">
                <div class="gt-sym-section-title">3D Extrusion</div>
                <div class="gt-sym-row">
                    <label class="gt-sym-label">Enabled</label>
                    <input type="checkbox" data-control="extToggle" ${extEnabled ? 'checked' : ''}
                        style="width:16px;height:16px;cursor:pointer">
                </div>
                <div class="gt-sym-row" data-section="extSliderRow" style="display:${extEnabled ? 'flex' : 'none'}">
                    <div class="gt-sym-slider-row">
                        <input type="range" class="gt-sym-slider" data-control="extScale"
                            min="50" max="500" step="10" value="${Math.round(extMult * 100)}">
                        <span class="gt-sym-value" data-display="extScale">${extMult.toFixed(1)}×</span>
                    </div>
                </div>
            </div>
        `;
  ...
  _bindEvents(overlay) {
    ...
    if (attrSelect) {
      attrSelect.addEventListener('change', (e) => {
        ...
        // Re-apply extrusion so it isn't lost when the style recompiles.
        if (this._extrusionScale > 0) {
          const renderer = this._getLayerData()?.renderer;
          renderer?.setExtrusionScale?.(this._extrusionScale);
        }
      });
    }
    ...
    // Extrusion toggle + slider
    const extToggle = overlay.querySelector('[data-control="extToggle"]');
    const extSliderRow = overlay.querySelector('[data-section="extSliderRow"]');
    const extSlider = overlay.querySelector('[data-control="extScale"]');
    const extDisplay = overlay.querySelector('[data-display="extScale"]');

    extToggle.addEventListener('change', () => {
      const renderer = this._getLayerData()?.renderer;
      if (extToggle.checked) {
        const mult = parseInt(extSlider.value) / 100;
        this._extrusionScale = 0.012 * mult;
        extSliderRow.style.display = 'flex';
      } else {
        this._extrusionScale = 0;
        extSliderRow.style.display = 'none';
      }
      renderer?.setExtrusionScale?.(this._extrusionScale);
    });
    ...
    extSlider.addEventListener('input', () => {
      const mult = parseInt(extSlider.value) / 100;
      extDisplay.textContent = `${mult.toFixed(1)}×`;
      if (extToggle.checked) {
        this._extrusionScale = 0.012 * mult;
        const renderer = this._getLayerData()?.renderer;
        renderer?.setExtrusionScale?.(this._extrusionScale);
    ...
    overlay.querySelector('[data-control="reset"]').addEventListener('click', () => {
      ...
      extSliderRow.style.display = 'none';
      // Rebuild discrete color list so stale custom colors don't persist.
      this._initCategoricalColors();
      this._rebuildDiscreteList(overlay);

      const renderer = this._getLayerData()?.renderer;
      renderer?.setExtrusionScale?.(0);
      this._applyUniformStyle();
    });
  }
