/**
 * DGFlexShards.js — Multi-file loader for temporally sharded DGFlex data.
 *
 ...
 * decode (RLE/sparse/dense/combined shards, mesh tiles, lazy column loading) is
 * relocated here VERBATIM behind the subclass — no decode logic was rewritten.
 *
 ...
 * Exposes the same data interface as DGFlexDecoder for seamless renderer integration.
 *
...
import { decodeDGFlex, decodeDGFMesh } from '../DGFlexDecoder.js';
import {
  fetchColumns,
  isShardV3,
  decodeShardV3,
...
export class DGFlexShards extends ShardLoader {
  /**
  ...
  constructor(manifestUrl, options = {}) {
    ...
    this.baseData = null; // Decoded DGFlex base (mesh + static)
  }
   ...
   * @returns {Promise<Object>} Decoded DGFlex data object
   */
  async load(activeMetric) {
    ...
    this.baseData = await decodeDGFlex(baseBuffer, this.manifest);
    ...
    if (this.baseData.mesh && this.baseData.mesh.vertexCount === 0) {
      ...
      if (this.manifest.meshTiles) {
        ...
        if (meshBuffer) {
          ...
          const ct = meshResp.headers.get('content-type') || '';
          if (ct.includes('text/html')) {
            throw new Error(
              `Mesh URL returned HTML instead of binary data — server likely ` +
                `served a SPA fallback for a missing route: ${meshUrl}`
            );
          }
          meshBuffer = await maybeDecompress(await meshResp.arrayBuffer());
          await putMeshInCache(meshUrl, meshBuffer);
        }
        this.baseData.mesh = decodeDGFMesh(meshBuffer);
      }
    }
  ...
  /**
   * Load spatially-sharded DGM2 mesh tiles with viewport-selective loading.
  ...
  async _loadMeshTiles(t0) {
    const INITIAL_BATCH = 15; // tiles for first render (~28MB)
    ...
    // 5. Split into initial (viewport) + remaining
    const initialTiles = tilesWithDist.slice(0, INITIAL_BATCH).map((t) => t.tile);
    const remainingTiles = tilesWithDist.slice(INITIAL_BATCH).map((t) => t.tile);

    console.debug(
      `[ShardedLoader] Loading ${initialTiles.length} viewport tiles first, ${remainingTiles.length} in background`
    );

    // 6. Fetch + decode initial batch → build mesh
    this._loadedTiles = [];
    const initialResults = await this._fetchAndDecodeTiles(initialTiles);
    this._loadedTiles.push(...initialResults);
    ...
    // 7. Background-load remaining tiles — each goes directly to GPU
    if (remainingTiles.length > 0) {
      this._backgroundLoadTiles(remainingTiles).catch((err) => {
        console.error('[ShardedLoader] Fatal error during background tile loading:', err);
      });
    }
  }
  ...
  async _fetchAndDecodeTiles(tiles) {
    ...
    const results = await Promise.all(
      tiles.map(async (tile) => {
        ...
        if (buffer) {
          ...
          if (!resp.ok) throw new Error(`Tile ${tile.index}: ${resp.status}`);
          const ct = resp.headers.get('content-type') || '';
          if (ct.includes('text/html')) {
            throw new Error(
              `Mesh tile returned HTML instead of binary data — server likely ` +
                `served a SPA fallback for a missing route: ${tileUrl}`
            );
          }
    ...
    return results.map(({ tile, buffer }) => {
      const mesh = decodeDGFMesh(buffer);
   ...
   *   - decodeDGFMesh creates typed array views (zero-copy pointer arithmetic)
  ...
  async _backgroundLoadTiles(tiles) {
    ...
    for (let i = 0; i < tiles.length; i += CONCURRENCY) {
      ...
      console.debug(
        `[ShardedLoader] Background chunk ${i / CONCURRENCY + 1} loaded → ${finalMesh.vertexCount.toLocaleString()} verts`
      );
    }
  }
  ...
  async switchMetric(metricName) {
    ...
    // Combined shards (explicit or implied by v3 root-level shards)
    const isEffectivelyCombined =
      this._isCombined ||
      (this.manifest.version >= 3 &&
        this.manifest.temporalAttributes?.length > 0 &&
        !this.manifest.temporalAttributes[0].shards);

    if (isEffectivelyCombined) {
  ...
  /**
   * Lazily load a single temporal column into the active shard.
   * Called by ChartDataAdapter when a chart references a non-active metric.
   * Fetches only the requested column — no impact on the active metric.
   * @param {string} columnName - Temporal attribute to load
   * @returns {Promise<Float32Array|null>}
   */
  async loadColumn(columnName) {
    const shardIdx = this._activeShardIdx;
    if (shardIdx < 0) return null;

    // Already loaded in the shard Map?
    const shard = this._shards.get(shardIdx);
    if (shard instanceof Map && shard.has(columnName)) {
      const data = shard.get(columnName);
      // Promote into temporalColumns for the renderer/chart fast path
      if (this.baseData?.temporalColumns) {
        this.baseData.temporalColumns[columnName] = data;
      }
      return data;
    }

    // Deduplicate: don't re-fetch if already in flight
    const pendingKey = `col_${shardIdx}_${columnName}`;
    if (this._pendingFetches.has(pendingKey)) return null;

    const shardList = this._getShardList();
    const shardInfo = shardList[shardIdx];
    const url = this.baseUrl + shardInfo.file;

    this._pendingFetches.set(pendingKey, true);
    try {
      const result = await fetchColumns(url, [columnName]);
      if (result?.columns?.has(columnName)) {
        const colBuf = result.columns.get(columnName);
        const decoded = this._unpackColumn(
          colBuf,
          result.types.get(columnName),
          shardInfo.epochCount
        );
        // Store in shard Map
        if (shard instanceof Map) {
          shard.set(columnName, decoded);
        }
        // Promote into temporalColumns
        if (this.baseData?.temporalColumns) {
          this.baseData.temporalColumns[columnName] = decoded;
        }
        console.debug(`[ShardedLoader] Lazy-loaded column "${columnName}" for shard ${shardIdx}`);
        return decoded;
      }
    } catch (err) {
      console.warn(`[ShardedLoader] Failed to lazy-load column "${columnName}":`, err);
    } finally {
      this._pendingFetches.delete(pendingKey);
    }
    return null;
  }
