/**
 * H3FlexRenderer.js — WebGPU implementation of H3Flex hexagonal rendering.
 *
...
import {
  computeEpochWindow,
  sliceEpoch,
  fillFloat32FromColumn,
  defaultH3StyleSpec,
} from './H3EpochUtils.js';
import { splitMercatorMesh } from '../util/mercatorBake.js';
import hexWGSL from './shaders/h3hex.wgsl?raw';
import hexMercWGSL from './shaders/h3hex.merc.wgsl?raw';
...
// This makes `extrusion: 0.012` look roughly the same size in both projections.
const MERC_EXTRUSION_FACTOR = 256 / (2 * Math.PI);
// Headroom factor for Mercator GPU buffers: antimeridian-crossing triangles
// emit up to 6 extra vertices each, but in practice only O(tens) of hexes
// straddle ±180°.  1% headroom (min 1024) is generous.
const MERC_SPLIT_HEADROOM = 0.01;
const MERC_SPLIT_MIN = 1024;

export class H3FlexRenderer {
  /**
   * Build the renderer. Constructs BOTH the spherical (`render()`) and the
   * Mercator (`renderMercator()`) pipelines up-front so projection toggles
   * are zero-cost at runtime. Mesh positions are baked in both formats:
   * ECEF for spherical, zoom-0 world pixels for Mercator (memory cost: ~2×
   * per-layer vertex buffer). Bind groups for data textures and the color
   * ramp are shared between the two pipelines.
   * @param {GPUDevice} device
   * @param {string} format - Canvas texture format
   * @param {string} depthFormat - Depth texture format
   * @param {Object} data - Decoded H3Flex data from H3FlexDecoder
  ...
  constructor(device, format, depthFormat, data, compiledStyle) {
    ...
    if (!data.mesh) {
      throw new Error('[H3FlexRenderer] H3Flex data has no pre-computed mesh.');
    }
  ...
  _directWriteToTex(texture, globalEpoch) {
    ...
    if (this._shardIsStatic) {
      ...
      if (attrData) buf.set(attrData.subarray(0, Math.min(cellCount, attrData.length)));
    } else {
      const attrData = this.data.temporalColumns?.[colorAttr];
      if (attrData) {
        const slice = sliceEpoch(attrData, globalEpoch, cellCount, this.data, colorAttr);
        buf.set(slice);
      }
    }
    this.device.queue.writeTexture(
  ...
  _buildMesh(maxVertexCount, maxIndexCount) {
    ...
    // vertices.  This keeps the pre-bake win (zero per-frame projection
    // cost) while eliminating the continent-spanning stretched triangle bug.
    // Pass extrudeFlags through the split so top vertices retain their flag.
    ...
    // Separate from cellIndexBuffer because split vertices are duplicates
    // with the same cell index but different world-X positions.
    this._mercCellIndexBuffer = device.createBuffer({
      label: 'H3Flex merc cell indices',
    ...
    this._mercExtrudeBuffer = device.createBuffer({
      label: 'H3Flex merc extrude flags',
      size: allocMercVerts * 4,
      usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
    });
    ...
    // Write-head pointers for incremental appends (diverge from spherical
    // offsets when split triangles emit extra vertices).
   ...
   *
   * The Mercator buffers are written via splitMercatorBatch(), which performs
  ...
  appendMeshData(
    ...
    // Keep Mercator buffers in sync — use split-aware helper so any
    // antimeridian-crossing triangles in this batch are handled correctly.
    // _mercVertexWriteHead and _mercIndexWriteHead track where to write
    // in the Mercator buffers (they diverge from spherical offsets when
    // split triangles add extra vertices).
    if (this._mercPositionBuffer && this._mercIndexBuffer) {
      // Re-base incoming indices to be relative to this batch's start,
      // then re-absolute them relative to the Mercator write head.
      ...
      // Shift mercIndices to be absolute Mercator vertex indices.
  ...
  _buildDataTextures() {
    ...
    this.dataTexA = createR32F('H3Flex data A');
    this.dataTexB = createR32F('H3Flex data B');
  ...
  prepareCompute(commandEncoder, normalizedTime) {
    const { e0, e1, frac } = computeEpochWindow(normalizedTime, this.epochCount);
    ...
    if (e0 !== this._currentEpoch) {
      if (this._currentEpoch < 0) {
        ...
        // Non-sequential scrub or shard transition: upload both
        this._directWriteToTex(this._texCurrent, e0);
        this._directWriteToTex(this._texNext, e1);
      }
  ...
  _uploadFilterColumn(columnName) {
    ...
    if (!colData) {
      ...
      if (colData && colData.length > this.cellCount) {
        ...
        const off = epoch * this.cellCount;
        colData = colData.subarray(off, off + this.cellCount);
      }
    }
    if (!colData) return;

    fillFloat32FromColumn(colData, this._filterTexBuf, this.cellCount);
  ...
  _buildMercPipeline() {
    ...
    // Uniform BGL — same structure as spherical (single uniform buffer), different content
    this._mercUniformBGL = device.createBindGroupLayout({
      label: 'H3Flex merc uniform BGL',
      entries: [
        {
          binding: 0,
          visibility: GPUShaderStage.VERTEX | GPUShaderStage.FRAGMENT,
  ...
  _renderMercator(ctx) {
    ...
    // Epoch management (same as render())
    const frac = this._computedFrac ?? 0;
    ...
    // world_size, tex_size, camera_offset (vec2), viewport_size (vec2), domain (vec2)
    f32[0] = worldSize;
    f32[1] = this._texSize;
    f32[2] = cameraX;
    f32[3] = cameraY;
    ...
    f32[6] = liveDomain[0];
    f32[7] = liveDomain[1];
    // epoch_frac, opacity, color_mode, _pad_a
    f32[8] = frac;
    f32[9] = liveOpacity;
    i32[10] = colorMode;
    i32[11] = 0;
    // filter_combinator, filter1_op, filter1_value, filter1_high, filter1_target
    const preds = this._filterPredicates;
    ...
    if (preds && preds.length > 0) {
      ...
      i32[13] = 0; // filter1_op = none
      i32[17] = 0; // filter2_op = none
    }
    ...
    // Use the Mercator-specific cell index buffer — it's parallel to
    // _mercPositionBuffer and includes indices for split antimeridian vertices.
    ...
    // Use the Mercator-specific index buffer (may reference split vertices).
